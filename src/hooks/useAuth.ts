      ...
      unsubscribeFromFirestore = onSnapshot(userDocRef, async (docSnapshot) => {
        if (docSnapshot.exists()) {
          ...
          try {
            ...
            // --- CORREÇÃO AQUI ---
            // Cria o objeto base do novo utilizador
            const newUser: Omit<User, 'uid' | 'accessUntil'> & { accessUntil?: Timestamp } = {
                email: newUserEmail,
            ...
            // Adiciona o campo 'accessUntil' apenas se ele tiver um valor válido
            if (accessUntil) {
                newUser.accessUntil = accessUntil;
            }
