...
export const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Se o login/registo for bem-sucedido, o listener do useAuth irá redirecionar
    } catch(err) {
    ...
    // Não é necessário um 'finally' aqui, pois o loading só deve parar se houver erro.
    // Em caso de sucesso, a aplicação irá re-renderizar de qualquer forma.
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setGoogleLoading(false); // Garante que o loading é desativado em caso de erro
    }
  };
